...
import {
  ...
  getAllEngagementMeta,
} from "src/store/selectors/features/all-engagements-selector";
...
export const AllEngagementListing = ({}: props) => {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const data = useSelector(getAllEngagementList);
  const meta: any = useSelector(getAllEngagementMeta);
  const isLoading = useSelector(getAllEngagementLoading);
  ...
  const handleGetEngagements = (
    ...
    dispatch(
      RequestAppAction.handleGetAllEngagements({ data, cbSuccess: func })
    );
  };
